...
class MainMenu(Menu):
	def __init__(self, game):
		...
		self.opsionx, self.opsiony = self.mid_w, self.mid_h +50
	...
	def display_menu(self):
		...
		while self.run_display:
			...
			self.game.draw_text('Start Menu', 20,self.startx, self.starty)
			self.game.draw_text('Options', 20,self.opsionx, self.opsiony)
	...
	def move_cursor(self):
		if self.game.DOWN_KEY:
			if self.state =='Start':
				self.cursor_rect.midtop = (self.opsionx + self.offset, self.opsiony)
				self.state = 'Options'
